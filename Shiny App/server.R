...
function(input, output, session) {
  ...
  ##### This is all text in reference to our one-sample t-test
  output$plot2 <- renderPrint({
      t <- t.test(x = our_data$woba_value/our_data$woba_denom, mu = .315)
      "Ho: The 2018 MLB Regular Season mean wOBA is equal to the 2018 MLB Playoff mean wOBA."
  })
  output$plot3 <- renderPrint({
      "Ha: The 2018 MLB Regular Season mean wOBA is not equal to the 2018 MLB Playoff mean wOBA."
  })
  ...
  output$plot7 <- renderPrint({
      "p-value = 0.04903"
  })
  output$plot8 <- renderPrint({
      "95% Confidence Interval: (0.27639, 0.31491)"
  })
  output$plot9 <- renderPrint({
    "We are 95% Confident that the true mean wOBA of the 2018 MLB Playoffs is different than true mean of the 2018 MLB Regular Season wOBA since our p-value, 0.0493, is less than alpha = 0.05"
  })
