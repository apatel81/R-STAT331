...
#shing.rstudio.com
#2 parts to every shiny app - user interface and server(where works get done)
#UI - Header, Sidebar, Main Panels - sky == limit
...
ui <- pageWithSidebar(
  headerPanel("Post-Season wOBA"),
  sidebarPanel(
    selectInput('player', 'Player Name', levels(our_data$player_name)),
    selectInput('opp_team', 'Opposing Team', levels(our_data$opp_team)),
    selectInput('pitch_type', 'Pitch Type', levels(our_data$pitch_type))
  ),
  mainPanel(
    plotOutput("plot1")
  )
...
server <- function(input, output, session) {
  ...
  output$plot1 <- renderPlot ({ 
    ...
    our_data %>%
      ggplot(aes(x = x_zone, y = z_zone, fill = bin_woba)) +
      geom_tile() +
      scale_fill_gradient(low = 'white', high = 'red')
  })
}  
